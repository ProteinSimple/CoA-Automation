...
def fill_CoA(
    config: dict, info: dict, trav_data: dict, model: str,
    write_path: Path | str = None
) -> Path:
    """Using the given template and fields yaml file in the directory,
       creates a new CoA, either in the specified path or in the same
       directory, and fills the fields given the appropriate command.
       for specs on COMMANDS see : exec_c

    Args:
        - dir_path: Path to the Directory containing the fields.yaml and
                    PDF template
            ...
            for field in page.widgets():
                if field.field_name == name:
                    ...
                    if fn in dates:
                        parts = val.split("/")
                        field.field_value = (
                            parts[1] + MONTH_MAP[int(parts[0])] + parts[2]
                        )
                    else:
...
def output_CoA(config, info, input_path, dest_filename):
    """Outputs the filled CoA to the path specified using the two
       passed arguments
...
def output_CoA_mapping(config, mapping: pd.DataFrame,
                       mapping_f_name="mapping.csv"):
    """Outputs the mapping data to path specified in config

    Args:
        config: Config dict containing 'mapping_output_dir' list of directory
                paths
